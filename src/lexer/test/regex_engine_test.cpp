...
void run_tests() {
  std::vector<Test> tests = {
      ...
      // Comentarios
      {"//.*\n?", "'a w2133 '",
       false},  // Esto tiene un problema y es que .* acepta \n, por lo que
      {"//.*\n?", "//dwlf w '",
       true},  // reconoceria todo el documento restante como un comentario :(
      {"/[*].*[*]/", "/*s r efl3el \n elkmf*/", true},
  ...
  for (size_t i = 0; i < tests.size(); ++i) {
    ...
    if (ok == tests[i].exp)
      ...
      std::cout << "Test " << i + 1 << " FALLO: /" << tests[i].pat << "/ vs \""
                << tests[i].txt << "\" esperaba=" << tests[i].exp << "\n";
  }
  std::cout << "Resultados: " << passed << "/" << tests.size()
            << " tests pasaron\n";
